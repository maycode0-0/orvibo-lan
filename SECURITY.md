...
该集成通过 Orvibo 云端获取设备拓扑，并在局域网连接 MixPad TCP 8088。属性型 WiFi 门锁还会由 Home Assistant 出站连接 Orvibo TCP 10002 接收实时事件，不需要对公网开放本机端口。不要把 8088 暴露到公网，也不要关闭 HTTPS 或 10002 的服务器证书校验。开放 Home Assistant 8123 不等于开放 MixPad 8088，但必须检查路由器端口转发、DMZ、UPnP/NAT-PMP 和 IPv6 入站规则。
...
10002 长连接使用厂商提供的双向 TLS 客户端证书，并固定 `china.orvibo.com` 服务端证书指纹；只有指纹校验完成后才发送 `readtable` 返回的临时账号和密码。临时凭据、会话密钥和原始事件只保存在内存中，断线后自动重连，不写入日志或诊断资料。
